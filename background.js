...
function isIcloudMailHost(hostname = '') {
  if (typeof navigationUtils !== 'undefined' && navigationUtils?.isIcloudMailHost) {
    return navigationUtils.isIcloudMailHost(hostname);
  }
  return hostname === 'www.icloud.com'
...
function matchesSourceUrlFamily(source, candidateUrl, referenceUrl) {
  ...
  switch (source) {
    ...
    case 'icloud-mail':
      return isIcloudMailHost(candidate.hostname);
    case 'gmail-mail':
...
async function runOpenAiCookieCleanup(options = {}) {
  const step = Math.max(1, Math.floor(Number(options.step) || 0)) || 6;
  const waitMs = Math.max(0, Math.floor(Number(options.waitMs) || 0));
  const startMessage = String(options.startMessage || '').trim();
  const successActionLabel = String(options.successActionLabel || '').trim();

  if (startMessage) {
    await addLog(startMessage, 'info');
  } else if (waitMs > 0) {
    await addLog(
      `步骤 ${step}：开始前等待 ${Math.round(waitMs / 1000)} 秒，然后直接删除 ChatGPT / OpenAI cookies...`,
      'info'
    );
  } else {
    await addLog(`步骤 ${step}：开始前快速清理 ChatGPT / OpenAI cookies，避免旧会话干扰。`, 'info');
  }
  ...
  await addLog(
    `步骤 ${step}：已直接删除 ${removedCount} 个 ChatGPT / OpenAI cookies${successActionLabel ? `，${successActionLabel}` : '。'}`,
    'ok'
  );
  return removedCount;
}
...
async function runPreStep1CookieCleanup() {
  return runOpenAiCookieCleanup({
    step: 1,
    startMessage: '步骤 1：开始前快速清理 ChatGPT / OpenAI cookies，避免旧会话干扰注册入口...',
    successActionLabel: '准备继续打开 ChatGPT 官网。',
  });
}
...
async function runPreStep6CookieCleanup() {
  return runOpenAiCookieCleanup({
    ...
    successActionLabel: '准备继续获取链接并登录。',
  });
}
