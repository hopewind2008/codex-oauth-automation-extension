(function attachSignupFlowHelpers(root, factory) {
  ...
  function createSignupFlowHelpers(deps = {}) {
    ...
    async function ensureSignupEntryPageReady(step = 1, options = {}) {
      const readinessTimeoutMs = Math.max(5000, Number(options?.readinessTimeoutMs) || 30000);
      const transportTimeoutMs = Math.max(
        readinessTimeoutMs + 5000,
        Number(options?.transportTimeoutMs) || 0,
      ) || 35000;
      ...
      const result = await sendToContentScriptResilient('signup-page', {
        ...
        responseTimeoutMs: Math.max(30000, readinessTimeoutMs + 5000),
      });
