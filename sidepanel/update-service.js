(() => {
  ...
  const VERSION_FAMILY_PLAIN = 'plain';
  const VERSION_FAMILY_MILESTONE = 'milestone';
  ...
  function getVersionFamily(version, fallbackFamily = VERSION_FAMILY_LEGACY) {
    ...
    if (/^m/i.test(trimmed)) {
      return VERSION_FAMILY_MILESTONE;
    }
  ...
  function stripVersionPrefix(version) {
    return String(version || '').trim().replace(/^(?:pro|v|m)\s*/i, '');
  }
  ...
  function getVersionFamilyRank(family) {
    ...
    if (family === VERSION_FAMILY_MILESTONE) {
      return 3;
    }
