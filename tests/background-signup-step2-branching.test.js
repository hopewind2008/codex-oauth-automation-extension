...
test('signup flow helper forwards custom signup entry readiness timeout to content script', async () => {
  const helperCalls = [];
  ...
  const helpers = signupFlowApi.createSignupFlowHelpers({
    ...
    sendToContentScriptResilient: async (_source, message, options) => {
      helperCalls.push({ message, options });
  ...
  const result = await helpers.ensureSignupEntryPageReady(1, {
  ...
  assert.deepStrictEqual(result, {
    tabId: 41,
    result: { ready: true, state: 'entry_home', url: 'https://chatgpt.com/' },
  });
  assert.deepStrictEqual(helperCalls, [
    {
      message: {
      ...
      options: {
        timeoutMs: 50000,
        retryDelayMs: 900,
        logMessage: '步骤 1：官网注册入口正在切换，等待页面恢复...',
...
test('signup flow helper opens signup entry tab without eager injection and lets readiness loop retry injection', async () => {
  ...
  const helpers = signupFlowApi.createSignupFlowHelpers({
    buildGeneratedAliasEmail: () => '',
    chrome: { tabs: { get: async () => ({ id: 51, url: 'https://chatgpt.com/' }) } },
    ensureContentScriptReadyOnTab: async (_source, tabId, options) => {
      ensureCalls.push({ tabId, options });
    },
    ensureHotmailAccountForFlow: async () => ({}),
    ensureLuckmailPurchaseForFlow: async () => ({}),
    isGeneratedAliasProvider: () => false,
    ...
    isSignupEmailVerificationPageUrl: () => false,
    isSignupPasswordPageUrl: () => false,
    reuseOrCreateTab: async (...args) => {
      reuseCalls.push(args);
      return 51;
    },
    sendToContentScriptResilient: async () => ({ ready: true, state: 'entry_home', url: 'https://chatgpt.com/' }),
    setEmailState: async () => {},
    SIGNUP_ENTRY_URL: 'https://chatgpt.com/',
    SIGNUP_PAGE_INJECT_FILES: ['content/utils.js', 'content/signup-page.js'],
  ...
  assert.deepStrictEqual(ensureCalls, [
    {
      ...
      options: {
        ...
        timeoutMs: 45000,
        retryDelayMs: 900,
        logMessage: '步骤 1：ChatGPT 官网仍在加载，正在重试连接内容脚本...',
      },
    },
  ]);
});
